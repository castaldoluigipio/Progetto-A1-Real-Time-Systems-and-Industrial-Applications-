...
RESULTS_FILE = "benchmark_results_gpu.csv"
...
def get_hardware_metrics():
    ...
    # 2. LETTURA POTENZA GPU
    try:
        hwmon_base_dir = "/sys/class/hwmon"
        if os.path.exists(hwmon_base_dir):
            found_gpu_rail = False
            
            # Scansiona tutte le cartelle hwmon presenti (hwmon0, hwmon1, hwmon2, ecc.)
            for hdir in os.listdir(hwmon_base_dir):
                path_h = os.path.join(hwmon_base_dir, hdir)
                
                # Su Jetson Orin, il chip INA3221 espone solitamente fino a 3 canali (in1, in2, in3)
                for i in range(1, 4):
                    label_path = os.path.join(path_h, f"in{i}_label")
                    
                    if os.path.exists(label_path):
                        with open(label_path, "r") as f_lbl:
                            label = f_lbl.read().strip().upper()
                        
                        # Se la label contiene "GPU" (es. VDD_GPU o VDD_GPU_SOC), abbiamo trovato la rotaia giusta
                        if "GPU" in label:
                            v_gpu_path = os.path.join(path_h, f"in{i}_input")
                            i_gpu_path = os.path.join(path_h, f"curr{i}_input")
                            
                            if os.path.exists(v_gpu_path) and os.path.exists(i_gpu_path):
                                with open(v_gpu_path, "r") as f_v, open(i_gpu_path, "r") as f_i:
                                    # inX_input è in mV, currX_input è in mA
                                    # (mV * mA) / 1000.0 = milliwatt (mW)
...
def run_benchmark(model_display_name, model_path, task_name, task_info, temperature, stressor_name, stress_cmd=None):
    ...
    cmd = [
        ...
        "--seed", "42", "-no-cnv", "--temp", str(temperature), "-ngl", "99"
    ]
